...
class Firm:
    ...
    depreciation_rate: float = 0.035

    def decide_labor_demand(self, wage: float) -> float:
        if wage <= 0:
            self.labor_demand = 0.0
        else:
            marginal_product = (1 - self.alpha) * self.productivity * (self.capital ** self.alpha)
            self.labor_demand = (marginal_product / wage) ** (1 / self.alpha)
        return self.labor_demand * self.n
